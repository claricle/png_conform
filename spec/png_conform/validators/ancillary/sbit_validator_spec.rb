...
require "spec_helper"
require "png_conform/validators/ancillary/sbit_validator"

RSpec.describe PngConform::Validators::Ancillary::SbitValidator do
  ...
  describe "#validate" do
    context "with valid SBIT chunk" do
      it "accepts valid data" do
        chunk = create_chunk("sbit", [8, 8, 8].pack("CCC"))
    ...
    context "multiple SBIT chunks" do
      before do
        context.record_chunk("sBIT")
      end

      it "rejects multiple SBIT chunks" do
        chunk = create_chunk("sbit", [8, 8, 8].pack("CCC"))
        described_class.new(chunk, context).validate
